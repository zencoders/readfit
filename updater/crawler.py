...
class _Fetcher(HTMLParser):

    def set_handling_function(self,func,reset=False):
        if reset or (self.__url_list is None):
            self.__url_list=[]
...
class Crawler:
    ...
    def crawl(self):
        ...
        return self.__fetcher.get_url_list()
...
def __reddit_fetch_function(tag,attr):
	if tag=="a":
		if 'href' in attr:
			if 'class' in attr:
